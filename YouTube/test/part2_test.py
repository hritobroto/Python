...
def test_add_to_playlist(capfd):
    ...
    lines = out.splitlines()
    assert len(lines) == 2
    assert "Cannot create playlist: A playlist with the same name already exists" in lines[0]
...
def test_add_to_playlist_already_added(capfd):
    ...
    assert len(lines) == 3
    assert "Cannot create playlist: A playlist with the same name already exists" in lines[0]
    assert "Cannot add video to my_cool_playlist: Video already added" in lines[1]
...
def test_add_to_playlist_nonexistent_playlist_nonexistent_video(capfd):
    ...
    lines = out.splitlines()
    assert len(lines) == 1
    assert "Cannot add video to my_cool_playlist: Video does not exist" in lines[0]
...
def test_show_all_playlists_no_playlists_exist(capfd):
    ...
    lines = out.splitlines()
    assert len(lines) == 3
    assert "Showing all playlists:" in lines[0]
    assert "my_PLAYlist" in lines[1]
    assert "my_cool_playlist" in lines[2]
...
def test_show_all_playlists(capfd):
    ...
    assert "my_PLAYlist" in lines[3]
    assert "my_cool_playlist" in lines[4]
    assert "anotheR_playlist" in lines[5]
...
def test_show_playlist(capfd):
    ...
    assert "Amazing Cats (amazing_cats_video_id) [#cat #animal]" in lines[2]
    assert "Cannot add video to my_cool_playlist: Video already added" in lines[3]
...
def test_show_playlist_nonexistent_playlist(capfd):
    ...
    lines = out.splitlines()
    assert len(lines) == 1
    assert "No videos here yet" in lines[0]
...
def test_remove_from_playlist_nonexistent_playlist(capfd):
    ...
    lines = out.splitlines()
    assert len(lines) == 1
    assert "Removed video from my_cool_playlist: Amazing Cats" in lines[0]
...
def test_clear_playlist_nonexistent(capfd):
    ...
    lines = out.splitlines()
    assert len(lines) == 1
    assert "Successfully removed all videos from my_cool_playlist" in lines[0]
